...
// ─── Generic Modal — iOS-style sheet that covers the screen ──
// Slides up from the bottom, dims+blurs everything behind, top-left "Annuler"
// and optional top-right primary action. Tap backdrop / Escape to close.
function Modal({ open, title, subtitle, onClose, onPrimary, primaryLabel, primaryDisabled, children }) {
  React.useEffect(() => {
    if (!open) return;
  ...
  return (
    <div onClick={onClose} style={{
      ...
      animation: 'hm-modal-fade 220ms ease-out',
    }}>
      <div onClick={(e) => e.stopPropagation()} style={{
        width: '100%', height: 'calc(100% - 36px)',
        background: HEMEA.bg,
        borderTopLeftRadius: 22, borderTopRightRadius: 22,
        boxShadow: '0 -10px 30px rgba(18,38,63,.20)',
        animation: 'hm-modal-slide 320ms cubic-bezier(0.22, 0.61, 0.36, 1)',
        display: 'flex', flexDirection: 'column', overflow: 'hidden',
      }}>
        {/* Grabber */}
        <div style={{ width: 38, height: 4, borderRadius: 999, background: HEMEA.border, margin: '6px auto 0' }}/>
        ...
        <div style={{
          ...
          <button onClick={onClose} style={{
            ...
            color: HEMEA.primary, fontSize: 15, fontWeight: 500, fontFamily: 'inherit',
          }}>Annuler</button>
          <div style={{ flex: 1, textAlign: 'center', minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: HEMEA.text, letterSpacing: '-0.01em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{title}</div>
            {subtitle && <div style={{ fontSize: 11, color: HEMEA.muted, marginTop: 1 }}>{subtitle}</div>}
          </div>
          {onPrimary ? (
            <button onClick={onPrimary} disabled={primaryDisabled} style={{
              background: 'transparent', border: 'none', padding: '4px 0',
              cursor: primaryDisabled ? 'not-allowed' : 'pointer',
              color: primaryDisabled ? HEMEA.muted : HEMEA.primary,
              fontSize: 15, fontWeight: 700, fontFamily: 'inherit',
            }}>{primaryLabel || 'OK'}</button>
          ) : <div style={{ width: 60 }}/>}
        </div>
      ...
      <style>{`
        @keyframes hm-modal-fade { from { opacity: 0; } to { opacity: 1; } }
        @keyframes hm-modal-slide { from { transform: translateY(60px); opacity: 0.6; } to { transform: translateY(0); opacity: 1; } }
      `}</style>
    </div>
  );
}
...
Object.assign(window, {
  ...
  AnalysisCard, StatCard, ListRow, ListSection, Banner, Modal,
});
