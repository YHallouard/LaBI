import { IOSMultipeerSyncService } from '../IOSMultipeerSyncService';
...
describe('IOSMultipeerSyncService', () => {
  let service: IOSMultipeerSyncService;
  let RNMultipeer: any;
  ...
  beforeEach(() => {
    ...
    service = new IOSMultipeerSyncService();
  });
  ...
  it('should throw error if initialized on non-iOS platform', async () => {
    // Override Platform.OS
    (Platform as any).OS = 'android';
    
    await expect(service.initialize()).rejects.toThrow('IOSMultipeerSyncService can only be used on iOS devices');
