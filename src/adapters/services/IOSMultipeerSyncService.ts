...
export class IOSMultipeerSyncService implements SyncingServicePort {
  ...
  async initialize(): Promise<void> {
    if (Platform.OS !== 'ios') {
      throw new Error('IOSMultipeerSyncService can only be used on iOS devices');
    }
    ...
    try {
      ...
      console.error('Error initializing iOS multipeer service:', error);
      throw error;
    }
  }
  ...
  async startAdvertising(deviceName: string): Promise<void> {
    ...
    // Enhance the device name with iOS device model
    ...
    if (isExpo) {
      ...
      enhancedDeviceName = `${deviceName} (iOS Simulator)`;
    } else {
      try {
        // Only import DeviceInfo dynamically when not in Expo Go
        const DeviceInfo = require('react-native-device-info');
        const model = await DeviceInfo.getModel();
        enhancedDeviceName = `${deviceName} (${model})`;
      } catch (error) {
        console.log("Could not get iOS device model:", error);
      }
    }
