...
export class SyncDevicesUseCase {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      try {
        const { InMemoryMultipeerSyncService } = require('../../adapters/services/InMemoryMultipeerSyncService');
        this.syncingService = new InMemoryMultipeerSyncService();
        await this.syncingService.initialize();
        this.setupEventListeners();
  ...
  async startAsReceiver(): Promise<void> {
    ...
    this.syncingService.setAutoDataReceptionCallback(async (data: any) => {
      console.log('[SyncDevicesUseCase] Auto data reception triggered, starting import process...');
      try {
        await this.performDataImport(data);
      } catch (error) {
        console.error('[SyncDevicesUseCase] Error in auto data import:', error);
        this.notifyProgress(SyncStatus.ERROR, 0, `Failed to import received data: ${error}`);
      }
    });
  ...
  async startSync(): Promise<boolean> {
    try {
      console.log('[SyncDevicesUseCase] startSync called with role:', this.currentRole);
      
      if (this.currentRole === SyncRole.SENDER) {
        console.log('[SyncDevicesUseCase] Taking SENDER path - calling sendData()');
        return await this.sendData();
      } else if (this.currentRole === SyncRole.RECEIVER) {
        console.log('[SyncDevicesUseCase] Taking RECEIVER path - calling receiveData()');
        return await this.receiveData();
      } else {
        console.error('[SyncDevicesUseCase] No role set! currentRole is:', this.currentRole);
  ...
  async stopSync(): Promise<void> {
    console.log('[SyncDevicesUseCase] Stopping sync and cleaning up session...');
  ...
  private async sendData(): Promise<boolean> {
    try {
      console.log('[SyncDevicesUseCase] sendData called - preparing to export data');
      ...
      console.log('[SyncDevicesUseCase] Data exported:', {
        ...
        dataKeys: allData && typeof allData === 'object' ? Object.keys(allData) : 'not an object',
        tablesCount: allData?.tables ? Object.keys(allData.tables).length : 'no tables property',
    ...
    if (data.user_profile && Array.isArray(data.user_profile)) {
      for (const profile of data.user_profile) {
        if (profile.profileImage && typeof profile.profileImage === 'string') {
          try {
            console.log('[SyncDevicesUseCase] Converting profile image to base64:', profile.profileImage);
            this.notifyProgress(SyncStatus.TRANSFERRING, 25, "Processing profile image");
            
            // Check if file exists
            const fileInfo = await FileSystem.getInfoAsync(profile.profileImage);
            if (fileInfo.exists) {
              // Detect image format from file extension
              const imageFormat = this.getImageFormat(profile.profileImage);
              
              // Read file as base64
              const base64Data = await FileSystem.readAsStringAsync(profile.profileImage, {
              ...
              console.log('[SyncDevicesUseCase] Profile image converted to base64 successfully');
            } else {
              console.warn('[SyncDevicesUseCase] Profile image file does not exist:', profile.profileImage);
              profile.profileImage = null;
            }
          } catch (error) {
            console.error('[SyncDevicesUseCase] Error converting profile image to base64:', error);
            profile.profileImage = null;
          }
        }
      }
  ...
  private async performDataImport(receivedData: any): Promise<void> {
    ...
    console.log('[SyncDevicesUseCase] Data to import:', {
      ...
      dataKeys: receivedData && typeof receivedData === 'object' ? Object.keys(receivedData) : 'not an object',
    ...
    try {
      ...
      if (receivedData.biological_analyses && Array.isArray(receivedData.biological_analyses)) {
        this.notifyProgress(SyncStatus.TRANSFERRING, 30, "Importing biological analyses");
        await this.importBiologicalAnalyses(receivedData.biological_analyses);
      }
      
      // Import user profile using repository
      if (receivedData.user_profile && Array.isArray(receivedData.user_profile) && receivedData.user_profile.length > 0) {
        this.notifyProgress(SyncStatus.TRANSFERRING, 60, "Importing user profile");
      ...
      console.error('[SyncDevicesUseCase] Error during repository-based import:', error);
      throw error;
    }
  }

  private async importBiologicalAnalyses(analysesData: any[]): Promise<void> {
    console.log(`[SyncDevicesUseCase] Importing ${analysesData.length} biological analyses`);
      ...
      try {
        ...
        if (analysisData.lab_values) {
          try {
            ...
            console.warn(`Failed to parse lab values for analysis ${analysisData.id}:`, parseError);
          }
        }
        ...
        console.error(`[SyncDevicesUseCase] Failed to save analysis ${analysisData.id}:`, error);
        throw error;
      }
    }
  }
  ...
  private async convertBase64ToFile(base64Data: string): Promise<string | null> {
    try {
      ...
      console.error('[SyncDevicesUseCase] Error converting base64 to file:', error);
      return null;
    }
  }
  ...
  private async performPostSyncCleanup(): Promise<void> {
    try {
      ...
      console.error('[SyncDevicesUseCase] Error during post-sync cleanup:', error);
    }
  }
