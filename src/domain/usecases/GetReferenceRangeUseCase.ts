...
export class GetReferenceRangeUseCase {
  ...
  getForAnalysis(analysis: BiologicalAnalysis, labKey: string): ReferenceRange {
    return this.execute(labKey, analysis.date);
  }

  generateChartRangesByDates(labKey: string, dates: Date[]): ReferenceRange[] {
    return dates.map((date) => this.execute(labKey, date));
  }
}
