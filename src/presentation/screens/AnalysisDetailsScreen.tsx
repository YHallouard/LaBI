...
interface AnalysisDetailsScreenProps extends StackScreenProps<HomeStackParamList, 'AnalysisDetails'> {
...
const AnalysisDetailsScreen: React.FC<AnalysisDetailsScreenProps> = ({ 
  ...
  function renderLabValueField(key: string) {
    ...
    const isActive = isEditing ? activeMetrics[key] : 
      displayValue !== undefined && displayValue !== null;
    
    const refRange = LAB_VALUE_REFERENCE_RANGES[key];
    const isOutOfRange = isActive && displayValue && refRange && 
            ...
            <View style={styles.valueRow}>
              {isEditing && isActive ? (
                ...
                <Text style={[styles.labValueText, isOutOfRange ? styles.outOfRangeText : null]}>
                  {displayValue.value != null ? displayValue.value.toFixed(2) : '0.00'} {displayValue.unit || LAB_VALUE_UNITS[key] || ''}
                </Text>
              ) : null}
            </View>
  ...
  <View>
    ...
    >
      <View style={styles.editableDateContainer}>
        <Ionicons name="calendar-outline" size={18} color="#2c7be5" style={{ marginRight: 8 }} />
        <Text style={styles.date}>{formattedDate}</Text>
      </View>
    </TouchableOpacity>
